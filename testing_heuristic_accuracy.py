...
correct_times = np.zeros((len(heuristic_num), len(heuristic_prob)))
...
for i, w in enumerate(heuristic_num):
  for j, s in enumerate(heuristic_prob):
      ...
      correct_time = correct_count/ 10
      print("The run time is ", correct_time)
      correct_times[i, j] = correct_time
      print(f"Word size: {w}, Seq length: {s}, Time: {correct_time:.4f} s")
...
# Plot the surface
surf = ax.plot_surface(W, S, correct_times, cmap='viridis', edgecolor='none')
